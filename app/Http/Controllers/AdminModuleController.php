...
class AdminModuleController extends Controller
{
    public function index(Request $request)
    {
        ...
        $created = DB::table('documents')->selectRaw("id as document_id, id as event_id, 'Created' as action, created_at as occurred_at, created_by as employee_id, origin_station_id as station_id, NULL as from_station_id, NULL as remarks");
        $released = DB::table('document_movements')->whereNotNull('released_at')
            ->selectRaw("document_id, id as event_id, 'Released' as action, released_at as occurred_at, released_by as employee_id, to_station_id as station_id, from_station_id, remarks");
        $received = DB::table('document_movements')->whereNotNull('received_at')
            ->selectRaw("document_id, id as event_id, 'Received' as action, received_at as occurred_at, received_by as employee_id, to_station_id as station_id, from_station_id, remarks");
        ...
        $activities = DB::query()->fromSub($created->unionAll($released)->unionAll($received), 'events')
            ...
            ->select('events.*', 'd.tracking_number', 'd.title', 'e.first_name', 'e.middle_name', 'e.last_name', 's.name as station_name', 'f.name as from_station_name')
